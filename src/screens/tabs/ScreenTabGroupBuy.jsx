...
export default function ScreenTabGroupBuy() {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: T.gray50 }} className="scrollbar-hide">
      ...
      {/* 배너 */}
      <div style={{ background: BANNERS[bannerIdx].grad, flexShrink: 0 }}>
        <div style={{ padding: '20px 20px 20px', textAlign: 'center' }}>
          ...
          <p style={{ color: T.white, fontSize: 20, fontWeight: 800, lineHeight: 1.3, whiteSpace: 'pre-line', marginBottom: 6, letterSpacing: -0.5 }}>{BANNERS[bannerIdx].title}</p>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 12, fontWeight: 500, marginBottom: 14 }}>{BANNERS[bannerIdx].sub}</p>
          <button
            onClick={() => BANNERS[bannerIdx].action && setScreen(BANNERS[bannerIdx].action)}
            style={{ background: T.white, color: T.gray800, border: 'none', borderRadius: 9999, padding: '8px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: T.shadowMd }}
      ...
      <div style={{ background: T.white, borderBottom: `1px solid ${T.gray100}` }}>
        <div style={{ display: 'flex', overflowX: 'auto', padding: '10px 12px 10px', gap: 8, msOverflowStyle: 'none', scrollbarWidth: 'none' }} className="scrollbar-hide">
          {CATEGORIES.map(c => {
            const on = activeCategory === c.key;
            return (
              <button key={c.key} onClick={() => setCategory(c.key)} style={{
                flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                padding: '7px 12px', borderRadius: 12, border: `1px solid ${on ? T.brand + '40' : T.gray100}`,
      ...
      {/* 인기 셀러 */}
      <div style={{ background: T.white, marginBottom: 8, padding: '14px 0 12px' }}>
      ...
      {/* 공구 리스트 */}
      <div style={{ background: T.white, padding: '14px 14px 24px' }}>
